...
begin // <<<<<<<<<<<<<<<<<<<<<<<<<<<<
  ...
  //    GetTag(HTML, '<', '>', iPos, Tag);
      ...
      if sTag = 'table' then
      begin
        if sTagClass='texts' then fParse := true; // ищем таблицу с классом texts
      end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        begin
          dImgs[i].URL := 'http://' + http.Request.Host +dImgs[i].URL; { TODO 4 : проверить!! }
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Imgs.Add(dImgs[i].name); // список путей файлов картинок для отправки с письмом
  ...
  begin
    ...
    begin
      ...
      dImgs[i].name := sm + GenName + '.' + extimg; // новое имя картинки (ID)
      Imgs.Add(dImgs[i].name); // список путей файлов картинок для отправки с письмом
      sHTML := StringReplace(sHTML, dImgs[i].URL, dImgs[i].name, [rfIgnoreCase]); // замена URL на путь к файлу картинки
