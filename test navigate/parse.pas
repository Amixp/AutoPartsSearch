...
begin // <<<<<<<<<<<<<<<<<<<<<<<<<<<<
  ...
  try
    repeat
      ...
      { DONE 5 : Найти способ вытащить текст между тегами }
      ...
      begin
        if sTagClass = 'texts' then
          fParse := true; // ищем таблицу с классом texts
      end;
...
var
  ...
  m: TMemoryStream;
  // Buff: array of byte;
  { DONE : Добавить удаление изображений }
  { TODO : Добавить проверку на одинаковые изображения }
begin
  P := 1;
  I := 0;
  ...
  Tag.params := TStringList.Create;
  // память для загрузки картинки с сервера
  ...
  begin
    ...
    try
      ...
      begin
        SetLength(dImgs, I + 1);
        // выделим память под новую запись
        GetTag(sHTML, '<', '>', P, Tag);
        ...
        // список путей файлов картинок для отправки с письмом
        sHTML := StringReplace(sHTML, dImgs[I].URL, dImgs[I].name,
          [rfIgnoreCase]);
        // замена URL на путь к файлу картинки
        ST.Clear;
        try
          { TODO 666: Проверить обработку!! делать обертку для обработки исключений }
          GetImg(dImgs[I].URL, ST); // загрузка картинки в память
          // http.Get(dImgs[i].URL, ST); // загрузка картинки в память
        except
          on e: Exception do
            FormMain.Log('Error sending message: ' + e.message, 3);
          // on e: EIdHTTPProtocolException do FormMain.Log('Ошибка: ' + e.Message + '. URL:' + dImgs[i].URL, 3);
        end;
        SetLength(dImgs[I].img, ST.Size);
        // выделить память в массиве под картинку
        ST.Position := 0;
        ST.Read(dImgs[I].img[0], ST.Size);
        // загрузка в массив из памяти
        inc(I);
      end;
    finally
      ST.free;
  ...
  begin
    ...
    try
      { DONE 3 :  ошибка загрузки последующей картинки!
...
var
  ...
  m: TMemoryStream;
begin
  P := 1; // глобальная позиция в тексте
  I := 0; // индекс массива картинок
  sm := ExtractFilePath(Application.ExeName) + 'img\';
  Tag.params := TStringList.Create;
  ST := TMemoryStream.Create; // память для загрузки картинки с сервера
  { TODO : Разбить функцию на несколько малых }
  // --- вставка стиля по ссылке ---
  ...
  // http.Request.Accept:='image/png, image/svg+xml, image/*;q=0.8, */*;q=0.5';
  // http.Request.Referer:='http://www.bl-recycle.jp/servlet/EW3SAS_WEB_PS?E_WORKID=ID1510&AD9500=40255073&AD3000=2000';
  P := 1; { TODO 111: Проверить! Почему тут два блока процедуры поиска тегов с href и <a }
  while P > 0 do // цикл до конца буфера или пока не кончатся теги картинки
  begin
    P := PosEx('<a', sHTML, P); // ищеем картинку в скрипте
    if P > 0 then
    begin
      GetTag(sHTML, '<', '>', P, Tag);
      S := Tag.params.Values['href'];
      ...
      begin
        ...
        // замена URL на путь к файлу картинки
        { TODO : добавить парсер расширения файла картинки }
        ST.Clear; { TODO : парсер доп ссылок }
        { TODO : Сделать обертку для обработки исключений }
        // GetHTTP(http, dImgs[i].URL, st); // загрузка картинки в память
        GetImg(dImgs[I].URL, ST); // загрузка картинки в память
        { TODO 665: Проверить GetImg }
        // выделить память в массиве под картинку
        ST.Position := 0;
        if ST.Size > 0 then
        begin
  ...
  begin
    ...
    begin
      ...
      // ссылка на картинку
      // dImgs[i].name  := Copy(dImgs[i].url,  LastDelimiter('/', dImgs[i].url)+ 1, MaxInt);
      dImgs[I].name := sm + GenName + '.' + extimg; // новое имя картинки (ID)
      imgs.Add(dImgs[I].name);
      // список путей файлов картинок для отправки с письмом
      sHTML := StringReplace(sHTML, dImgs[I].URL, dImgs[I].name, [rfIgnoreCase]
        ); // замена URL на путь к файлу картинки
      { DONE : добавить парсер расширения файла картинки }
      ST.Clear; { TODO : парсер доп ссылок }
      { TODO : Сделать обертку для обработки исключений }
...
function TParseHTML.ParserScript(var sData: TStrings; sScript: string;
  http: TIdHTTP): boolean; (*
  парсер скрипта - разбирание членов выражения на элементы списка
*)
var
          ...
          begin
            ...
            begin
              StrGrd.Cells[iColumn, iLine] := sHTTP + StrGrd.Cells
                [iColumn, iLine]; { TODO 5 : !!! проверить!!! }
            end;
          end;
        end;
      end;
...
begin // <<<----------
  try
    ...
    begin
      ...
      begin
        if iCountTag = 0 then
          break; // найден закрывающий тег = Выход
        dec(iCountTag);
      end;
      if pos('br', S) > 0 then // перевод строки на пробел
        sk := sk + ' ';
    ...
    // копируем все содержимое тега с его именем
    Result := true;
  except
    on e: Exception do
      FormMain.Log('Error sending message: ' + e.message, 3);
  ...
  try
    ...
    begin
      ...
      begin
        ...
        // выбор табличных данных с начала и до конца позиций
      end;
    end;
  finally
...
{ function ParseTable1(HTML: string; StrGrd: TStringGrid): integer;
  ...
  while Tag.name <> '' do
  begin
  if Tag.name = td then      { TODO : Сделать парсер!!! }
