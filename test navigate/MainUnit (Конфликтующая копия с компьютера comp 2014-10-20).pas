...
uses
  ...
  System.Classes, Vcl.Graphics, mshtml,  httpsend,
  ...
  IdLogDebug, IdCoder, IdCoderQuotedPrintable, IdIntercept, IdIOHandler,
  IdIOHandlerSocket, IdIOHandlerStack, IdSSL, IdSSLOpenSSL, IdLogStream;
...
uses parse, getstrparam, UnitSend;

procedure TFormMain.WB_LoadHTML(WebBrowser: TWebBrowser; HTMLCode: string);
...
procedure TFormMain.WebBrowser1BeforeNavigate2(ASender: TObject; const pDisp: IDispatch;
  const URL, Flags, TargetFrameName, PostData, Headers: OleVariant; var Cancel: WordBool);
...
procedure TFormMain.WebBrowser1DownloadBegin(Sender: TObject);
begin
  fDownload := true;
end;

procedure TFormMain.WebBrowser1DownloadComplete(Sender: TObject);
begin
  fDownload := false;
end;

procedure TFormMain.AddPost(sPost: string);
var
  sl: Tstrings;
...
begin
  ...
  try
    ...
    Data.Add('AD1210=' + edLogin.Text);
    Data.Add('AD1510=' + edpass1.Text);
    Data.Add('AD1520=' + edpass2.Text);
    ...
    StrPage := PostHTTP('POST',http, edurl.Text, Data);
    ...
    begin
      ...
      PostHTTP('POST',http, edurl.Text, Data);
      ...
      begin
        ...
        { TODO : Сделать обетрку для обработки исключений }
        StrPage := PostHTTP('POST',http, edurl.Text, Data) + EOL + '============================' + EOL;
        Log('Парсим таблицу');
          ...
          begin
            ...
            begin
              ...
              try
                ...
                Log('Отсылаем письмо.');
                SendMail2('a.cia@yandex.ru', 'a.cia@yandex.ru', 'smtp.yandex.ru', '465', 'New parts.', 'a.cia',
                  'Ferdy4to)', StrPage, Imgs);
                DeleteImgs(Imgs);
                { DONE : Проверить почту }
                { DONE : Добавить работу с базой запчастей для контроля новых }
...
procedure TFormMain.DeleteImgs(listFiles: Tstrings);
var
  i: Integer;
begin
  i := 0;
...
begin
  sURL := edurl.Text;
  sPost := 'AD1210=' + edlogin.Text + '&AD1510=' + edpass1.Text + '&AD1520=' + edpass2.Text +
  ...
  PostWithWebBrowser(sPost, EdURL.Text, SHeader); // Login
...
begin
  ...
  EdSender := 'artem-xp@yandex.ru';
  EdRecipients := 'artem-xp@yandex.ru';
  with IdMessage do
  begin
    ...
    IsEncoded := true;
    Body.Text := WebBrowser1.LocationURL;
  end;
  Log('SendMail.');
  SendMail1(IdMessage);
...
const
  CRLF =#13#10;// в Интернете применяют #$0a + #$0d;
var
  EndOfPart,__EndOfPart: String; // их можно как константы объявлять, но Мозилла, как я понял, значения им дает "генератор-случайные"
  HTTP: THTTPSend;
  Res: boolean;
  d: TStringStream;
  s: AnsiString;
  Login,password,
  FilePrice: String; // путь к файлу для загрузки на сайт
  FS: TFileStream;
begin
{Memo1 - для html-текста страницы}
{Memo2 - просто лог}

{    --- Авторизация на auto.ru средствами Synapse (Delphi) ---   }
// данные авторизации, файл для загрузки
Login    :=Edit1.Text;
Password :=Edit2.Text;
FilePrice:=Edit8.Text;  // путь к локальному файлу, который потом загрузим на сайт

HTTP:=THTTPSend.Create;
HTTP.UserAgent:='Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 6.0)';
HTTP.MimeType:='application/x-www-form-urlencoded';
HTTP.Headers.Add('Accept-Encoding: gzip, deflate');
d:=TStringStream.Create('referer=http://koleso.auto.ru/office/?op=price_file&act=new&user_name='+Login+'&user_pass='+Password+'&enter=Войти');
HTTP.Document.LoadFromStream(d);
d.Free;
HTTP.Cookies.Add('chcookie=1;'); // без этого не работало!
//try
  Res:=HTTP.HTTPMethod('POST', 'https://auth.auto.ru/login.html');  // и без этих двух строк подряд не работает авторизация
  Res:=HTTP.HTTPMethod('POST', 'http://koleso.auto.ru/office/?op=price_file&act=new');
  Memo2.Lines.Add('Отправка запроса выполнена. Результат: '+BoolToStr(Res));
  Memo2.Lines.Add('Ответ сервера:');
  Memo2.Lines.Add('-----Cokies-----');
  Memo2.Lines.add(http.Cookies.Text);
  Memo2.Lines.Add('-----Headers-----');
  Memo2.Lines.add(http.Headers.Text);
  Memo2.Lines.Add('http.ResultCode='+IntToStr(http.ResultCode));
  Memo2.Lines.Add('http.MimeType='+http.MimeType);
  ...
  if pos('https://auth.auto.ru/logout.html',Memo1.Text)>0  // можно из http.document в  TStringStream скинуть данные, тогда можно искать эту строку не в Memo1.Text, а в StringStream.DataString
    then Memo2.Lines.Add('Авторизованы успешно!!!')//ShowMessage('Авторизовались')
    else
      Begin
      Memo2.Lines.Add('Не удалось авторизоваться'); //ShowMessage('Не удалось');
      HTTP.Free;
      Exit;
      End;
  http.Clear;

  {    --- Загрузка файла (прайса) на auto.ru средствами Synapse (Delphi)---   }
  // формируем данные для загрузки прайса на сайт (загрузка файла на сайт)
  HTTP.Headers.Clear;
  HTTP.Document.Clear;
  // разделитель в заголовках
  EndOfPart:='-----------------------------265001916915724'; // Можно любую комбинацию, главное, чтобы была уникальной на странице. Можно и произвольные числа, поэтому предлагаю тут переменную, а не константу
  __EndOfPart:='--'+EndOfPart; // то же, но с двумя дефисами слева! Они важны

  s:='';
  s:=s+__EndOfPart+CRLF+'Content-Disposition: form-data; name="MAX_FILE_SIZE"'+CRLF+CRLF+'5120000'+CRLF;  // посмотрел, что мозила отправляла, это же тут и ввожу
  s:=s+__EndOfPart+CRLF+'Content-Disposition: form-data; name="sale_type"'    +CRLF+CRLF+'tyre'+CRLF;
  s:=S+__EndOfPart+CRLF+'Content-Disposition: form-data; name="sale_delete"'  +CRLF+CRLF+'0'+CRLF;
  s:=S+__EndOfPart+CRLF+'Content-Disposition: form-data; name="price"; filename="autoru.txt"'+CRLF+'Content-Type: text/plain'+CRLF+CRLF;

  HTTP.Document.Write(PAnsiChar(s)^, Length(s));
  FS:=TFileStream.Create(FilePrice, fmOpenRead); // загружаем отправляемый файл (прайс) в поток памяти
  FS.Position := 0;
  // записываем файл в тело документа
  HTTP.Document.CopyFrom(FS, FS.Size);
  FS.Free;
  // завершаем тело запроса
  s:=CRLF+__EndOfPart+CRLF+'Content-Disposition: form-data; name="__submit__"'+CRLF+CRLF+'РЎРѕС…СЂР°РЅРёС‚СЊ'+CRLF+__EndOfPart+'--'+CRLF; // каракули можно культурно написать (Сохранить)
  HTTP.Document.Write(PAnsiChar(s)^, Length(s)); // завершили тело документа
  HTTP.Document.SaveToFile('111.txt'); // в данном файле можно посмотреть, что мы наформировали для отправки на сайт. Проге это не нужно, но посмотреть стоит
  HTTP.MimeType := 'multipart/form-data; boundary='+EndOfPart; // важный момент!
  Memo2.Lines.Add('-----Отправка файла-----');
  Memo2.Lines.Add('http.MimeType='+http.MimeType);
  // Отправляем запрос
  Res:=HTTP.HTTPMethod('POST', 'http://koleso.auto.ru/office/?op=price_file&act=new'); // тут файл и загружается на сайт
  Memo2.Lines.Add('Отправка запроса выполнена. Результат: '+BoolToStr(Res));
  Memo2.Lines.Add('Ответ сервера:');
  Memo2.Lines.Add('-----Cokies-----');
  Memo2.Lines.add(http.Cookies.Text);
  ...
  If http.ResultCode=200
    Then Memo2.Lines.Add('200 - неудачная отправка файла');
  If http.ResultCode=302
    Then Memo2.Lines.Add('302 - УСПЕШНАЯ отправка файла!!!');
  Memo2.Lines.Add('http.MimeType='+http.MimeType);
  Memo2.Lines.Add('http.ResultString='+http.ResultString);
  // визуальная проверка результата
  http.Clear;
  http.HTTPMethod('get','http://koleso.auto.ru/office/?op=price_file'); // откроем список загруженных прайсов
  Memo2.Lines.LoadFromStream(HTTP.Document); // перекинем HTML данные в Memo
  Memo2.Text:=Utf8ToAnsi(Memo2.Text);
  Memo2.Text:=trim(Memo2.Text);
  TextToWebBrowser(Memo2.Text,WebBrowser2); // загрузка html данных в TWebBrowser. Функция предложена lipskiy и Donal_Graeme в 2002 году, есть в сети.
  //finally
    HTTP.Free;
  //end;
end;
...
procedure TFormMain.PostWithWebBrowser(sPost: string; URL, Headers: OleVariant);
var
  Data: Pointer;
  i: Integer;
  Post: OleVariant;
...
procedure TFormMain.ListPArtsClick(Sender: TObject);
var
  F: TForm;
  WB: TWebBrowser;
  // L: TLabel;
...
procedure TFormMain.Log(Txt: string; debuglevel: Integer = 0; ToConsole: boolean = true);
begin
